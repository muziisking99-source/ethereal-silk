...
export default function ProductGalleryCard({
  ...
  return (
    <motion.div
      ...
      className={`group g-card flex-shrink-0 relative overflow-hidden cursor-pointer border border-[rgba(255,45,107,0.2)] rounded-[4px] transition-all duration-300 hover:shadow-[0_12px_40px_rgba(255,45,107,0.25)] hover:border-[#FF2D6B] bg-[#1F1F1F] ${variantClasses[variant]} ${className}`}
    >
      <img
        src={image}
        alt={product.name}
        ...
        loading="lazy"
      />
      <div className="absolute inset-0 backdrop-blur-[8px] bg-[rgba(13,13,13,0.7)] flex flex-col items-center justify-center gap-4 opacity-0 hover:opacity-100 transition-all duration-300">
        <button
          type="button"
          onClick={() =>
            addItem({
          ...
          className="w-[52px] h-[52px] rounded-full border border-[#FF2D6B] flex items-center justify-center bg-[#FF2D6B] hover:bg-white hover:border-white transition-all duration-300"
          aria-label={`Add ${product.name} to basket`}
        >
          <ShoppingBag className="w-5 h-5 text-white group-hover:text-[#FF2D6B]" strokeWidth={1.5} />
        </button>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-[2]">
        ...
        <div className="font-[DM_Mono] text-[0.55rem] tracking-[0.2em] uppercase text-[#FF2D6B] mt-0.5">
          {product.sku}
        </div>
      </div>
    </motion.div>
